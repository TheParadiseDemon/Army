...
namespace Container {
    ...
    public:
        ...
        friend std::ostream& operator<<(std::ostream& os, const forward_list<T>& obj)
        {
            ...
            for (auto i = obj.begin(); i._pointee; i++)
                os << *i << std::endl;
            return os;
        }
        ...
        _node_pointer pop_front(){
            ...
            if (_head == _tail)
                ...
                tmp = _head; // use the tmp to store the current _head, later, the node which _head points to will
...
#endif //FORWARD_LIST_CONTAINER_FORWARD_LIST_H
